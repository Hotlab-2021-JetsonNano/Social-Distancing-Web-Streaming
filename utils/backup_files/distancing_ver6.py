...
def distancing_algorithm(idTable, person1, person2):
    ## get distance info
...
def grouping_algorithm(img, config, groupCoordsList, peopleList, idTable):
...
def draw_polygons(img, color, groupCoordsList):
    overlay = img.copy()
    opacity = 0.5
    for polyCoords in groupCoordsList:
        if len(polyCoords) >= 2:
            polyCoords.sort(key=lambda x : x[0])
            polyCoords.sort(key=lambda x : (x[0], -x[1]))
            polyCoords = np.int32([np.array(polyCoords)])
             #hull = ConvexHull(np.array(polyCoords))
             #cv2.fillConvexPoly(overlay, [hull.vertices], color.red)
            cv2.fillConvexPoly(overlay, polyCoords, color.red)
            #cv2.polylines(overlay, polyCoords, False, color.red)

    cv2.addWeighted(overlay, opacity, img, 1 - opacity, 0, img)
    return img

def show_distancing(img, boxes, oldPeopleList, validIdList, invalidIdList):
    
    if len(boxes) == 0:
        return img, oldPeopleList, validIdList, invalidIdList
...
### Distancing Algorithm
    ...
    for index1, index2 in peopleIdxCombi:
        ...
        if is_valid(idTable, person1, person2):
            distancing_algorithm(idTable, person1, person2)

### Grouping Algorithm
    config = Configs(img)
    groupCoordsList = [[] for i in range(len(peopleList))]
    img = grouping_algorithm(img, config, groupCoordsList, peopleList, idTable)
    img = draw_polygons(img, config.get_colors(), groupCoordsList)
...
### Check Undetected people
    ...
    return img, peopleList, validIdList, invalidIdList
